...
class Inventario
{
	...
	public function agregarInventario()
	{
		...
		$sql="insert into inventariodamas(idElemento, nombElemento, cantElemento, mediElemento) values ('$this->codigo', '$this->nombre', '$this->cantidad', '$this->medida')";
	...
	public function restarInventario($id, $cantidadRestada)
	{
		...
		if ($resultadoResta>=0)
		{
			$sql="UPDATE inventariodamas SET elemCantidad = " . $resultadoResta ."WHERE idElemento= " . $idMerc;
			$resultado=$this->Conexion->query($sql);
		}
		else
		{
	...
	{
		$this->Conexion=Conectarse();
		$sql="select * from inventariodamas";
		$resultado=$this->Conexion->query($sql);
		$this->Conexion->close();
		return $resultado;
	}
	...
	{
		$this->Conexion=Conectarse();
		$sql="select * from inventariodamas where idElemento='$codigo'";
		$resultado=$this->Conexion->query($sql);
		$this->Conexion->close();
		return $resultado;
	}
	...
	{
		$this->Conexion=Conectarse();
		$sql="DELETE from inventariodamas where idElemento='$codigo'";
		$resultado=$this->Conexion->query($sql);
		$this->Conexion->close();
		return $resultado;
	}
