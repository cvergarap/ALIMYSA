...
class Inventario
{
	...
	public function agregarInventario()
	{
		...
		$sql="insert into inventariogeneral(idElemento, nombElemento, cantElemento, mediElemento) values ('$this->codigo', '$this->nombre', '$this->cantidad', '$this->medida')";
	...
	public function restarInventario($id, $cantidadRestada)
	{
		...
		if ($resultadoResta>=0)
		{
			$sql="UPDATE inventariogeneral SET elemCantidad = " . $resultadoResta ."WHERE idElemento= " . $idMerc;
			$resultado=$this->Conexion->query($sql);
		}
		else
		{
	...
	{
		$this->Conexion=Conectarse();
		$sql="select * from inventariogeneral";
		$resultado=$this->Conexion->query($sql);
		$this->Conexion->close();
		return $resultado;
	}
	...
	{
		$this->Conexion=Conectarse();
		$sql="select * from inventariogeneral where idElemento='$codigo'";
		$resultado=$this->Conexion->query($sql);
		$this->Conexion->close();
		return $resultado;
	}
	...
	{
		$this->Conexion=Conectarse();
		$sql="DELETE from inventariogeneral where idElemento='$codigo'";
		$resultado=$this->Conexion->query($sql);
		$this->Conexion->close();
		return $resultado;
	}
