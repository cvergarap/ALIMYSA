...
class Inventario
{
	...
	public function agregarInventario()
	{
		...
		$sql="insert into inventarioinfantil(idElemento, nombElemento, cantElemento, mediElemento) values ('$this->codigo', '$this->nombre', '$this->cantidad', '$this->medida')";
	...
	public function restarInventario($id, $cantidadRestada)
	{
		...
		if ($resultadoResta>=0)
		{
			$sql="UPDATE inventarioinfantil SET elemCantidad = " . $resultadoResta ."WHERE idElemento= " . $idMerc;
			$resultado=$this->Conexion->query($sql);
		}
		else
		{
	...
	{
		$this->Conexion=Conectarse();
		$sql="select * from inventarioinfantil";
		$resultado=$this->Conexion->query($sql);
		$this->Conexion->close();
		return $resultado;
	}
	...
	{
		$this->Conexion=Conectarse();
		$sql="select * from inventarioinfantil where idElemento='$codigo'";
		$resultado=$this->Conexion->query($sql);
		$this->Conexion->close();
		return $resultado;
	}
	...
	{
		$this->Conexion=Conectarse();
		$sql="DELETE from inventarioinfantil where idElemento='$codigo'";
		$resultado=$this->Conexion->query($sql);
		$this->Conexion->close();
		return $resultado;
	}
