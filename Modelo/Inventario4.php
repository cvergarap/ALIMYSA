...
class Inventario
{
	...
	public function agregarInventario()
	{
		...
		$sql="insert into inventariojovenes(idElemento, nombElemento, cantElemento, mediElemento) values ('$this->codigo', '$this->nombre', '$this->cantidad', '$this->medida')";
	...
	public function restarInventario($id, $cantidadRestada)
	{
		...
		if ($resultadoResta>=0)
		{
			$sql="UPDATE inventariojovenes SET elemCantidad = " . $resultadoResta ."WHERE idElemento= " . $idMerc;
			$resultado=$this->Conexion->query($sql);
		}
		else
		{
	...
	{
		$this->Conexion=Conectarse();
		$sql="select * from inventariojovenes";
		$resultado=$this->Conexion->query($sql);
		$this->Conexion->close();
		return $resultado;
	}
	...
	{
		$this->Conexion=Conectarse();
		$sql="select * from inventariojovenes where idElemento='$codigo'";
		$resultado=$this->Conexion->query($sql);
		$this->Conexion->close();
		return $resultado;
	}
	...
	{
		$this->Conexion=Conectarse();
		$sql="DELETE from inventariojovenes where idElemento='$codigo'";
		$resultado=$this->Conexion->query($sql);
		$this->Conexion->close();
		return $resultado;
	}
